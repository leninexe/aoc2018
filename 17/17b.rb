...
floating = area.select{|k,v| k[1] >= minYCmd && v == "|"}
still = area.select{|k,v| k[1] >= minYCmd && v == "~"}

print "Floating Water: #{floating.length}\n"
print "Still Water: #{still.length}"
print "Overall Water: #{floating.length + still.length}\n"